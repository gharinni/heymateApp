...
export default function LoginScreen({ navigation }) {
  ...
  const submit = async () => {
    if (mode === 'login') {
      ...
      if (phone.length !== 10) { Alert.alert('Error','Enter 10-digit phone'); return; }
    ...
    try {
      ...
      else Alert.alert('Error','Cannot connect.\n'+(e.message||''));
    ...
    <ScrollView style={{ flex:1, backgroundColor:C.bg }}
      ...
      {mode==='signup' && (
        <>
          ...
          <TextInput style={inp} placeholder="Enter your name"
            ...
            {[{v:'USER',l:'👤 Customer'},{v:'PROVIDER',l:'🔧 Provider'}].map(r => (
              <TouchableOpacity key={r.v} onPress={() => setRole(r.v)}
                style={{ flex:1, padding:14, borderRadius:12, alignItems:'center',
                  borderWidth:2, borderColor:role===r.v?C.success:C.border,
                  backgroundColor:role===r.v?'#0a2a0a':C.card }}>
                <Text style={{ color:role===r.v?C.success:C.muted, fontWeight:'700' }}>
                  {role===r.v?'✓ ':''}{r.l}
                </Text>
              </TouchableOpacity>
            ))}
      ...
      <TextInput style={inp} placeholder="10-digit phone"
