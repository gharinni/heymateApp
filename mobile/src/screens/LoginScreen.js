...
export default function LoginScreen({ navigation }) {
  ...
  const handleSubmit = async () => {
    ...
    if (mode === 'login') {
      ...
      if (!form.name.trim())               { Alert.alert('Error', 'Enter your full name'); return; }
      if (form.phone.trim().length !== 10) { Alert.alert('Error', 'Enter valid 10-digit phone'); return; }
      if (form.password.length < 6)        { Alert.alert('Error', 'Password must be at least 6 chars'); return; }
    }
    ...
    try {
      ...
      // Set timeout to avoid infinite spinner
      ...
      if (userData?.token) {
        ...
        const msg = data?.message || data?.error || data?.data?.message
          || (response.status === 401 ? 'Wrong phone/password'
      ...
      if (e.name === 'AbortError') {
        Alert.alert('⏱️ Timeout', 'Server took too long to respond.\nCheck your internet connection.');
      } else {
  ...
  return (
    ...
    >
      ...
      <View style={{ alignItems: 'center', marginBottom: 36 }}>
        ...
        <Text style={{ fontSize: 34, fontWeight: '800', color: c.primary, marginTop: 8 }}>HeyMate</Text>
        <Text style={{ color: c.textMuted, fontSize: 14, marginTop: 6 }}>One App · Any Task · Any Time</Text>
      </View>

      {/* Mode Toggle */}
      <View style={{ flexDirection: 'row', backgroundColor: c.card, borderRadius: 16,
        padding: 4, marginBottom: 24, borderWidth: 1, borderColor: c.border }}>
          ...
          <View style={{ flexDirection: 'row', gap: 10, marginBottom: 16 }}>
            {[{ v: 'USER', l: '👤 Customer' }, { v: 'PROVIDER', l: '🔧 Provider' }].map(r => (
              <TouchableOpacity key={r.v} onPress={() => set('role')(r.v)}
                ...
                <Text style={{ color: form.role === r.v ? c.success : c.textMuted,
      ...
      <TextInput
        style={{ borderWidth: 1.5, borderColor: c.border, borderRadius: 14, padding: 16,
          color: c.text, backgroundColor: c.card, marginBottom: 28, fontSize: 15 }}
      ...
      <TouchableOpacity
        ...
        disabled={loading}
        style={{ backgroundColor: loading ? c.textMuted : c.primary,
