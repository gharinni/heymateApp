...
const TAB_STYLE = {
  headerShown: false,
  tabBarStyle: { backgroundColor: '#1A1A2E', borderTopColor: '#2A2A3E', height: isWeb ? 60 : 56 },
...
function MainTabs() {
  // Read role from storage to decide tabs
  const [isProvider, setIsProvider] = useState(false);
  useEffect(() => {
    (async () => {
      try {
        let u = null;
        if (Platform.OS === 'web') u = localStorage.getItem('user');
        else { const AS = (await import('@react-native-async-storage/async-storage')).default; u = await AS.getItem('user'); }
        if (u) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        else { const AS = (await import('@react-native-async-storage/async-storage')).default; token = await AS.getItem('token'); }
